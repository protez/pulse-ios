...
CSS_IMAGE_FILES = FileList.new('./lib/twitter-bootstrap/img/*.png').pathmap('%{^./lib/twitter-bootstrap/img,./css/images}p')
...
rule(/^\.\/css\/images\// => [proc {|t| t.pathmap('%{^./css/images,./lib/twitter-bootstrap/img}p')}]) do |t|
  cp t.source, t.name
end